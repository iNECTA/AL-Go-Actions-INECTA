...
try {
    ...
    $settingsJson = @{
        country                     = "us"
        artifact                    = "//28//" # TODO: remove hardcoding
    ...
    # Retrieve all directories in the base folder
    $allDirectories = Get-ChildItem -Path $baseFolder -Directory 

    # Retrieve app names from the envFile
    $appNames = $envFile.Apps | ForEach-Object { $_.App }

    # Iterate over all directories
    foreach ($dir in $allDirectories) {
        # If the directory name is not in the appNames list, remove the directory
        if ($appNames -notcontains $dir.Name) {
